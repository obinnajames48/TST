...
import { ArrowDownToLine, ArrowUpFromLine, Plus, AlertCircle, CreditCard, Bitcoin, Building2, Wallet as WalletIcon, ShieldCheck } from "lucide-react";
import { toast } from "sonner";
import PageHeader from "@/components/app/PageHeader";
import StatCard from "@/components/app/StatCard";
import { useFetch } from "@/lib/useFetch";
import { getWallet, deposit, withdraw, listTransactions, kycUpload } from "@/lib/api";

export default function Wallet() {
  const { data: walletRaw, refetch } = useFetch(getWallet);
  const wallet = walletRaw;
  const { data: txsRaw, refetch: refetchTxs } = useFetch(listTransactions);
  const txs = txsRaw || [];
  ...
  const doDeposit = async () => {
    try {
      await deposit(depositAmt, method);
      toast.success(`$${depositAmt} deposited`);
      refetch(); refetchTxs();
  ...
  const doWithdraw = async () => {
    try {
      await withdraw(wdAmt);
      toast.success(`$${wdAmt} withdrawal queued`);
      refetch(); refetchTxs();
    } catch (e) { toast.error(e.message); }
  };
  ...
  return (
    <div data-testid="wallet-page" className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard label="Available balance" value={`$${wallet.balance.toLocaleString()}`} icon={WalletIcon} tone="lime" />
        <StatCard label="Pending (in matches)" value={`$${wallet.pending.toLocaleString()}`} icon={AlertCircle} tone="purple" />
        <StatCard label="Lifetime earned" value={`$${wallet.lifetime_earned.toLocaleString()}`} icon={ShieldCheck} tone="dark" />
      </div>
