  ...
  return (
    <div className="space-y-6" data-testid="admin-matches">
      <div>
        ...
        <p className="text-[13px] text-[#6B7280] mt-2">Monitor every duel across the platform — including the 3-minute MT5 login window before live trading begins.</p>
      </div>

      <Tabs value={tab} onValueChange={setTab} className="w-full">
        <TabsList className="bg-white border border-[#ECECEA] rounded-full p-1 inline-flex h-auto" data-testid="match-tabs">
          {["active", "inactive", "completed"].map((k) => (
            ...
            >
              ...
              <span className="ml-2 inline-flex items-center justify-center text-[10px] font-mono px-1.5 py-0.5 rounded-full bg-[#F5F5F2] text-[#0F0F12] data-[state=active]:bg-white/15 data-[state=active]:text-white">
                {grouped[k].length}
              </span>
            </TabsTrigger>
          ))}
...
function MatchTable({ rows, group, onVoid }) {
  if (!rows.length) {
    return (
      <div className="bg-white border border-[#ECECEA] rounded-2xl p-12 text-center">
        <div className="text-[13px] text-[#6B7280]">No matches in this category.</div>
      </div>
    );
  }
  return (
    <div className="bg-white border border-[#ECECEA] rounded-2xl overflow-x-auto" data-testid={`match-table-${group}`}>
        ...
        <tbody>
          {rows.map((d) => (
            <tr key={d.id} className="border-b border-[#F1F1EF] hover:bg-[#FAFAF7]">
              <td className="py-3 px-5 font-mono text-[#0F0F12] font-semibold truncate max-w-[120px]" title={d.id}>{d.id.slice(0, 8)}…</td>
...
function Mt5Monitor({ duel }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className={`font-mono text-[12px] font-semibold ${remaining === 0 ? "text-[#EF4444]" : remaining < 60 ? "text-[#EF4444]" : "text-[#0F0F12]"}`}>{minsStr}:{secsStr}</div>
...
function Pip({ ok, label }) {
  return (
    <span title={`Trader ${label} ${ok ? "confirmed" : "pending"}`} className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-[9px] font-bold ${ok ? "bg-[#10B981]/15 text-[#10B981]" : "bg-[#F3F4F6] text-[#9CA3AF]"}`}>{label}</span>
  );
}
